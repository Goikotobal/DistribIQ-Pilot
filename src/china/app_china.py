...
# 1. PAGE CONFIG
st.set_page_config(
    page_title="DistribIQ-China | 中国版",
    page_icon="src/china/logo.png",
    layout="wide"
)

# 2. IMPORTS - ✅ CHANGED: Import from agent_china
try:
    from agent_china import prepare_knowledge_base, solver_agent
except ImportError:
    from src.china.agent_china import prepare_knowledge_base, solver_agent
...
# --- MAIN CHAT --- ✅ UPDATED: China Edition branding
st.markdown('<div class="main-header">🧬 DistribIQ - China Edition</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">AI Decision Support for Ingredient Distribution | 中国版</div>', unsafe_allow_html=True)
st.divider()

if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "Hello! I am DistribIQ (China Edition, powered by Qwen-Max). I have access to the Product Master, Pricing Tiers, and Logistics data. How can I help?"}
    ]
